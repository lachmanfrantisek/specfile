...
class TagSource(Source):
    ...
    def _get_index(self) -> Optional[str]:
        ...
        """
        tokens = re.split(r"(\d+)", self.tag.name, maxsplit=1)
        if len(tokens) > 1:
            return tokens[1]
        return None
    ...
    def expanded_location(self) -> str:
        """Location of the source after expanding macros."""
        return self.tag.expanded_value

    @property
    def filename(self) -> str:
        """Literal filename of the source."""
        return Path(urllib.parse.urlsplit(self.tag.value).path).name

    @property
    def expanded_filename(self) -> str:
        """Filename of the source after expanding macros."""
        return Path(urllib.parse.urlsplit(self.tag.expanded_value).path).name

    @property
    def comments(self) -> Comments:
        """List of comments associated with the source."""
        return self.tag.comments
...
class ListSource(Source):
    ...
    Attributes:
        source: Source object that this source represents.
    """
    ...
    @location.setter
    def location(self, value: str) -> None:
        self.source.location = value

    @property
    def expanded_location(self) -> str:
        """Location of the source after expanding macros."""
        return self.source.expanded_location

    @property
    def filename(self) -> str:
        """Literal filename of the source."""
        return Path(urllib.parse.urlsplit(self.source.location).path).name

    @property
    def expanded_filename(self) -> str:
        """Filename of the source after expanding macros."""
        return Path(urllib.parse.urlsplit(self.source.expanded_location).path).name
...
class Sources:
    ...
    def __repr__(self) -> str:
        tags = repr(self.tags)
        sourcelists = repr(self.sourcelists)
    ...
    @overload
    def __getitem__(self, i: slice) -> "Sources":
        pass
    ...
    def _get_tag_format(self, reference: TagSource, index: int) -> Tuple[str, str]:
        ...
        diff = len(reference.tag.name) - len(name)
        if diff >= 0:
            return name, reference.tag._separator + " " * diff
        return name, reference.tag._separator[:diff] or ":"
    ...
    def _get_initial_tag_setup(self) -> Tuple[int, str, str]:
        ...
        """
        prefix = self.PREFIX.capitalize()
        return len(self.tags) if self.tags else 0, f"{prefix}0", ": "
    ...
    def _deduplicate_tag_names(self) -> None:
        ...
        for ts0, ts1 in zip(tag_sources, tag_sources[1:]):
            if ts1.index <= ts0.index:
                ts1.tag.name, ts1.tag._separator = self._get_tag_format(
                    ts0, ts0.index + 1
                )
    ...
    def insert(self, i: int, location: str) -> None:
        ...
        """
        if not self.allow_duplicates and location in self:
        ...
        elif self.sourcelists:
            self.sourcelists[-1].append(SourcelistEntry(location, Comments()))
        else:
...
class Patches(Sources):
    ...
    Attributes:
        tags: All spec file tags.
        sourcelists: List of all %patchlist sections.
